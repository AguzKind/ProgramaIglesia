...
namespace Prj_Interfaz
{
    public partial class FRM_Comuniones : Form
    {
        ...
        {
            FRP_Comuniones Form_Reportes = new FRP_Comuniones();
            Form_Reportes.Datos.Clear();
            dataGridView1.DataSource = Comuniones.tablaImpresion(mtc_COM.SelectionRange.Start.ToShortDateString(), mtc_COM.SelectionRange.End.ToShortDateString());
            for (int i = 0; i <= dataGridView1.Rows.Count - 1; i++)
            {
                Reportes_Comuniones Reportes = new Reportes_Comuniones();
