...
namespace Prj_Interfaz
{
    public partial class FRM_Misas : Form
    {
        ...
        {
            FRP_Misas Form_Reportes = new FRP_Misas();
            Form_Reportes.Datos.Clear();
            dataGridView1.DataSource = Misas.tablaImpresion(mtc_MIS.SelectionRange.Start.ToShortDateString(), mtc_MIS.SelectionRange.End.ToShortDateString());
            for (int i = 0; i <= dataGridView1.Rows.Count - 1; i++)
            {
                Reportes_Misas Reportes = new Reportes_Misas();
                Reportes.Fecha = (string)this.dataGridView1.Rows[i].Cells[0].Value;
                Reportes.Hora = (string)this.dataGridView1.Rows[i].Cells[1].Value;
                Reportes.Intencion = (string)this.dataGridView1.Rows[i].Cells[2].Value;
                Form_Reportes.Datos.Add(Reportes);
            }
            Form_Reportes.Show();
        }
    }
