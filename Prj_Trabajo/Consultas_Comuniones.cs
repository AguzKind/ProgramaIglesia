...
namespace Prj_Trabajo
{
    public class Consultas_Comuniones
    {
        ...
        public DataTable tablaImpresion(string fecha_inicio, string fecha_fin)//Metodo que devolvera un tabla con todos los datos de el evento dentro de un rango de fechas seleccionadas
        {
            ...
            int[] id_evento = id_eventos_varias_fechas(fecha_inicio, fecha_fin); // Array que recibira todos los id_evento de las misas que se relacionan con las comuniones

            // Creamos la estructura de la tabla_bautismos para que quede igual que el dgv de las comuniones
            string[] nombresColumnas = { "Fecha", "Hora" }; // Arrat que contiene los nombres de todas los columnas de el DataTable, este se utilizara para cargar las tablas en un bucle
            for (int i = 0; i < nombresColumnas.Length; i++) // Bucle que se utiliza para crear todas las columnas del DataTable con sus respectivos nombres dados por el array nombreColumnas
                tabla_para_impresion.Columns.Add(nombresColumnas[i]);

            for (int i = 0; i < id_evento.Length; i++) // Se hace por la cantidad de comuniones que haya en la fecha seleccionada
            {
                ...
                tabla_para_impresion.Rows.Add();  // Se agrega una fila a tabla_misas donde se cargara todos los datos de la comunion
        ...
        private int[] id_eventos_varias_fechas(string fecha_inicio, string fecha_fin)
        {
            ...
            id_eventos = funciones_db.Lectura_tabla_BD("SELECT id_evento FROM eventos WHERE id_tipo_evento = 6 AND fecha BETWEEN '" + fecha_inicio + "' AND '" + fecha_fin + "' order by (eventos.fecha) asc"); // Cargamos la tabla con todos los id_evento entre dos fecha y de un tipo de evento en especifico
