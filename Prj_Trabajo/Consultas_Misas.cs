...
namespace Prj_Trabajo
{
    public class Consultas_Misas
    {
        ...
        public DataTable tablaImpresion(string fecha_inicio, string fecha_fin)//Metodo que devolvera un tabla con todos los datos de el evento dentro de un rango de fechas seleccionadas
        {
            funciones_db.AbriCerrarBD(true);

            DataTable tabla_para_impresion = new DataTable();
            DataTable tabla_fecha_hora = new DataTable(); // Tabla que contendra todas la fechas y horas de cada una de los eventos individualmente

            int[] id_evento = id_eventos_varias_fechas(fecha_inicio, fecha_fin); // Array que recibira todos los id_evento de las misas que se relacionan con las misas

            // Creamos la estructura de la tabla_bautismos para que quede igual que el dgv de las misas
            string[] nombresColumnas = { "Fecha", "Hora", "Intencion" }; // Arrat que contiene los nombres de todas los columnas de el DataTable, este se utilizara para cargar las tablas en un bucle
            for (int i = 0; i < nombresColumnas.Length; i++) // Bucle que se utiliza para crear todas las columnas del DataTable con sus respectivos nombres dados por el array nombreColumnas
                tabla_para_impresion.Columns.Add(nombresColumnas[i]);

            for (int i = 0; i < id_evento.Length; i++) // Se hace por la cantidad de misas que haya en la fecha seleccionada
            {
                tabla_fecha_hora = funciones_db.Lectura_tabla_BD("SELECT fecha, hora_ingreso, observaciones FROM eventos WHERE id_evento = " + id_evento[i]); // Carga en la tabla la fecha y la hora de un evento en especifico
                tabla_para_impresion.Rows.Add();  // Se agrega una fila a tabla_misas donde se cargara todos los datos de la misa
                tabla_para_impresion.Rows[i]["Fecha"] = tabla_fecha_hora.Rows[0]["fecha"]; // Carga la fecha en la fila que marca la variable i en la tabla donde estan todos los eventos
                tabla_para_impresion.Rows[i]["Hora"] = tabla_fecha_hora.Rows[0]["hora_ingreso"]; // Carga la hora en la fila que marca la variable i en la tabla donde estan todos los eventos
                tabla_para_impresion.Rows[i]["Intencion"] = tabla_fecha_hora.Rows[0]["observaciones"]; // Carga las intenciones en la fila que marca la variable i en la tabla donde estan todos los eventos
            }
            funciones_db.AbriCerrarBD(false);
            return tabla_para_impresion;
        }
        private int[] id_eventos_varias_fechas(string fecha_inicio, string fecha_fin)
        {
            DataTable id_eventos = new DataTable();
            id_eventos = funciones_db.Lectura_tabla_BD("SELECT id_evento FROM eventos WHERE id_tipo_evento = 4 AND fecha BETWEEN '" + fecha_inicio + "' AND '" + fecha_fin + "' order by (eventos.fecha) asc"); // Cargamos la tabla con todos los id_evento entre dos fecha y de un tipo de evento en especifico
            int[] id_eventos_separados = new int[id_eventos.Rows.Count]; // Instanciamos el array con la cantidad de id_evento que hay es decir con cada una de los eventos
            for (int i = 0; i < id_eventos_separados.Length; i++)
                id_eventos_separados[i] = Convert.ToInt32(id_eventos.Rows[i]["id_evento"]);
            return id_eventos_separados;
        }
    }
}
